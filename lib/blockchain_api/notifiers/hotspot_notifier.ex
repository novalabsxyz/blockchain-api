defmodule BlockchainAPI.HotspotNotifier do
  ...
  def send_new_hotspot_notification(txn, type, ledger) do
    ...
    data = %{hotspot_address: Util.bin_to_string(map.address), owner: Util.bin_to_string(map.owner), hash: Util.bin_to_string(txn.hash)}
  ...
  def send_add_hotspot_failed(:timed_out, pending_gateway) do
    data = %{hotspot_address: pending_gateway.gateway, owner: pending_gateway.owner}
  ...
  def send_confirm_location_failed(pending_location) do
    data = %{hotspot_address: pending_location.gateway, owner: pending_location.owner}
