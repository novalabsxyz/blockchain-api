defmodule BlockchainAPI.Test.Factory do
  ...
  def transaction_factory(attrs) do
    ...
    %Transaction{
      type: Map.get(attrs, :type, default_type),
      block_height: insert(:block).height,
      hash: :crypto.strong_rand_bytes(32),
      status: "cleared"
    }
  ...
  def oui_transaction_factory do
    %OUITransaction{
      hash: insert(:transaction).hash,
      status: "cleared",
      owner: :crypto.strong_rand_bytes(32),
  ...
  def oui_transaction_factory(attrs) do
    txn = Map.get(attrs, :txn)
    %OUITransaction{
      hash: txn.hash,
      status: "cleared",
      owner: :crypto.strong_rand_bytes(32),
