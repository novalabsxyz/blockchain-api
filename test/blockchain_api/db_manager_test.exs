defmodule BlockchainAPI.QueryTest do
  ...
  describe "blocks" do
    ...
    test "create_block/1 with valid data creates a block" do
      assert {:ok, %Block{} = block} = Query.Block.create_block(@block_valid_attrs)
    ...
    test "create_block/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Query.Block.create_block(@block_invalid_attrs)
    end
  ...
  describe "transactions" do
    ...
    def transaction_fixture(attrs \\ %{}) do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, attrs)
      transaction
    end

    test "get_transaction!/1 returns the transaction with given hash" do
      transaction = transaction_fixture(@transaction_valid_attrs)
      assert Query.Transaction.get_transaction!(transaction.hash) == transaction
    end
    ...
    test "create_transaction/1 with invalid data returns error changeset" do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:error, %Ecto.Changeset{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_invalid_attrs)
    end
  end
  ...
  describe "coinbase_transactions" do
    ...
    def coinbase_fixture(attrs \\ %{}) do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:ok, %CoinbaseTransaction{} = coinbase} = Query.CoinbaseTransaction.create_coinbase(transaction.hash, attrs)
      coinbase
    end

    test "list_coinbase_transactions/0 returns all coinbase_transactions" do
      coinbase = coinbase_fixture(@valid_attrs)
      assert Query.CoinbaseTransaction.list_coinbase_transactions(@default_params).entries == [coinbase]
    end

    test "get_coinbase!/1 returns the coinbase with given hash" do
      coinbase = coinbase_fixture(@valid_attrs)
      assert Query.CoinbaseTransaction.get_coinbase!(coinbase.hash) == coinbase
    end
    ...
    test "create_coinbase/1 with invalid data returns error changeset" do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:error, %Ecto.Changeset{}} = Query.CoinbaseTransaction.create_coinbase(transaction.hash, @invalid_attrs)
    end
  ...
  describe "payment_transactions" do
    ...
    def payment_fixture(attrs \\ %{}) do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:ok, %PaymentTransaction{} = payment} = Query.PaymentTransaction.create_payment(transaction.hash, attrs)
      payment
    end

    test "list_payment_transactions/0 returns all payment_transactions" do
      payment = payment_fixture(@valid_attrs)
      assert Query.PaymentTransaction.list_payment_transactions(@default_params).entries == [payment]
    end

    test "get_payment!/1 returns the payment with given hash" do
      payment = payment_fixture(@valid_attrs)
      assert Query.PaymentTransaction.get_payment!(payment.hash) == payment
    end
    ...
    test "create_payment/1 with invalid data returns error changeset" do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:error, %Ecto.Changeset{}} = Query.PaymentTransaction.create_payment(transaction.hash, @invalid_attrs)
    end
  end
  ...
  describe "gateway_transactions" do
    ...
    def gateway_fixture(attrs \\ %{}) do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:ok, %GatewayTransaction{} = gateway} = Query.GatewayTransaction.create_gateway(transaction.hash, attrs)
      gateway
    end

    test "list_gateway_transactions/0 returns all add_gateway_transactions" do
      gateway = gateway_fixture(@valid_attrs)
      assert Query.GatewayTransaction.list_gateway_transactions(@default_params).entries == [gateway]
    end

    test "get_gateway!/1 returns the gateway with given hash" do
      gateway = gateway_fixture(@valid_attrs)
      assert Query.GatewayTransaction.get_gateway!(gateway.hash) == gateway
    end
    ...
    test "create_gateway/1 with invalid data returns error changeset" do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:error, %Ecto.Changeset{}} = Query.GatewayTransaction.create_gateway(transaction.hash, @invalid_attrs)
    end
  end
  ...
  describe "location_transactions" do
    ...
    def gateway_location_fixture(attrs \\ %{}) do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:ok, %LocationTransaction{} = gateway_location} = Query.LocationTransaction.create_location(transaction.hash, attrs)
      gateway_location
    end
    ...
    test "list_location_transactions/0 returns all assert_location_transactions" do
      ...
      assert Query.LocationTransaction.list_location_transactions(@default_params).entries == [gateway_location]
    end

    test "get_location!/1 returns the gateway_location with given hash" do
      gateway_location = gateway_location_fixture(@valid_attrs)
      assert Query.LocationTransaction.get_location!(gateway_location.hash) == gateway_location
    end
    ...
    test "create_location/1 with invalid data returns error changeset" do
      {:ok, block} = Query.Block.create_block(@block_valid_attrs)
      assert {:ok, %Transaction{} = transaction} = Query.Transaction.create_transaction(block.height, @transaction_valid_attrs)
      assert {:error, %Ecto.Changeset{}} = Query.LocationTransaction.create_location(transaction.hash, @invalid_attrs)
    end
  ...
  describe "accounts" do
    ...
    def account_fixture(attrs \\ %{}) do
      {:ok, account} = Query.Account.create_account(attrs)
      account
    end
    ...
    test "get_account!/1 returns account with given address" do
      account = account_fixture(@valid_attrs)
      assert Query.Account.get_account!(account.address) == account
    end
