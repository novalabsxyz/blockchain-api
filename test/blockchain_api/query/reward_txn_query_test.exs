defmodule BlockchainAPI.Query.RewardTxnTest do
  ...
  setup do
    account =
      Repo.insert!(%Account{name: "account0", balance: 100, address: "address0", fee: 1, nonce: 0})
  ...
  describe "get_from_last_week/0" do
    test "returns sum of rewards from last week", %{
      account: account,
      block: block,
      reward_txn: reward_txn0
    } do
    ...
    test "returns sum of rewards for multiple accounts", %{
      account: account0,
      block: block,
      reward_txn: reward_txn0
    } do
      ...
      [reward0, reward1] =
        Query.RewardTxn.get_from_last_week() |> Enum.sort(&(&1.account < &2.account))
    ...
    test "doesn't returns rewards from over a week ago", %{
      account: account,
      reward_txn: reward_txn1
    } do
